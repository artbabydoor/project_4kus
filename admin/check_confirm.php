...
if ($formid != $_POST['formid']) {
	...
	if ($_POST) {
		...
		$meSql = "INSERT INTO buy_detail (BT_name, BT_namelist) VALUES ('{$order_fullname}','{$order_namelist}') ";
		...
		if ($meQeury) {	
			...
			for ($i = 0; $i < count($_POST['qty']); $i++) {
				...
				$lineSql = "INSERT INTO buy_item (buy_type,buy_product_id,buy_qty,buy_price,buy_date,buy_order) VALUES ('{$product_type}','{$product_id}','{$buy_qty}','{$buy_price}',NOW(),'{$order_id}') ";
				mysql_query($lineSql);
			...
			echo "<script>window.location='buy_item.php?a=order'</script>";
			
		}else{
			mysql_close();
				/*header('location:index.php?a=orderfail');*/
				echo "<script>window.location='buy_item.php?a=orderfail'</script>";
		}
	}
}
